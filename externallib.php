...
require_once(dirname(__FILE__)."/../SyncSheet/SyncSheet.php");
...
/**
     ...
     */
    public static function get_pauta_parameters() {
        return new external_function_parameters(
                array('idcurso' => new external_value(PARAM_INT, 'ID do curso para o qual se deseja a pauta', VALUE_DEFAULT, 0))
        );
    }
     ...
     */
    public static function get_pauta($idcurso) {
        global $DB;
        // global $USER;
        ...
        $idcurso = 1;
        $pauta = new StdClass();
        $sql = "select a.sessionid, u.username, a.studentid, a.statusid  from {attendance_log} a 
                    inner join {attendance_sessions} on a.sessionid = {attendance_sessions}.id
                    inner join {user} u on a.studentid = u.id   
                        where {attendance_sessions}.attendanceid = ?";
        $id = $DB->get_records_sql($sql,[$idcurso]);
        ...
        $pauta->idcurso = '123';
        $pauta->frequencias = $frequencias;
        $pauta->notas = $notas;
        $pauta = json_encode($pauta);
     ...
     */
    public static function get_pauta_returns() {
        return new external_value(PARAM_TEXT, 'JSON com dados da pauta do curso');
    }
